...
function StudentDataEntry({ data, authSession }) {
  ...
  // Treatments available for selected Location
  ...
  // Check if ALL 4 Required Selections are Made
  const isHeaderSelectionComplete = Boolean(
    selectedLocation && selectedPlot && selectedObsDay && selectedTreatment
  );

  // Selected details labels for UI
  ...
  const handleSubmit = (e) => {
    ...
    if (!isHeaderSelectionComplete) {
      setSubmitError(
        "Please select Location, Plot, Observation Day, and Treatment."
      );
      return;
    }
      ...
      >
        ...
        {treatmentDesc && (
          ...
          >
            ...
            <span>
              <b>Treatment Details for {selectedTreatment}:</b> {treatmentDesc}
            </span>
          </div>
        )}
      </div>
            ...
            >
              <div>
                ...
                <p style={{ margin: 0, fontSize: "13px", color: "#64748b" }}>
                  Recording data for <b>{locationObj?.shortName}</b> →{" "}
                  <b>{plotObj?.name}</b> ({selectedTreatment}) on{" "}
                  <b>{selectedObsDay} DAY</b>
                </p>
              </div>
